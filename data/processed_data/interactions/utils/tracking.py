...
class TrackingArray:
    ...
    def assign(self, out_file, frame_start=0, frame_end=-1):
        ...
        with open(out_file, "w") as outfile:
            ...
            for chunk_frame_start in range(frame_start, frame_end, self._chunk_size):
                ...
                self._process_frame_end = (
                    frame_end if frame_end < chunk_end else chunk_end
                )

                # Store the processing chunked coordsinates array
                self._process_coords_array = self._frames_coords_array[
                    self._process_frame_start : self._process_frame_end, :, :, :
                ]

                if self._threads > 1:
                    with Pool(processes=self._threads) as pool:
                        for report_list in pool.starmap(
                            self._checkForInteractions, _yield_combinations()
                        ):
    ...
    def plot(
        ...
        frame_end = (
            frame_end - 1
            if frame_end >= self._frames_coords_array.shape[0]
            else frame_end
        )

        # Create dataframe of instances to plot per frame
        frame_instances_dataframe = self.processInteractionsFile(
            interactions_filename,
            frame_start,
            frame_end,
        ...
        if plot_color == None:
            ...
            if len(plot_color) != 3:
                raise Exception(
                    f"Unable to assign plot color: {plot_color}. Iterables must be of length 3"
                )
        ...
        while frame_read and current_frame <= frame_end:
            ...
            instances_to_plot = frame_instances_dataframe[
                frame_instances_dataframe["Interaction Frame"] == current_frame
            ]["Instances"].values
            if len(instances_to_plot) > 1:
                raise Exception("Error parsing interactions for instances")
            if len(instances_to_plot) > 0:
                instances_to_plot = instances_to_plot[0]

            # Loop each instance for this frame
            for instance_to_plot in instances_to_plot:
                # Create an interactor using the instance
                instance_interactor = Interactor.withModelDict(
                    instance_to_plot,
                    current_frame,
                    self._nodes,
                    self._frames_coords_array[
                        current_frame, :, :, self._tracks.index(instance_to_plot)
                    ],
                    self._interactor_model_dict,
                )

                # Loop and plot each model for the instance
                for model_to_plot in models_to_plot:
                    instance_model_coords = int_coords(
                        instance_interactor._models["Head_w_antennae"].exterior.coords
                    )
                    cv2.polylines(
                        frame_img, [instance_model_coords], True, plot_color, line_width
                    )
    ...
    def _checkForInteractions(self, trackA, trackB):
        ...
        for frame, row in enumerate(filtered_coords_array):
            ...
            interactor1 = Interactor.withModelDict(
                self._tracks[trackA],
                adj_frame,
                self._nodes,
                row[:, :, 0],
            ...
            interactor2 = Interactor.withModelDict(
                self._tracks[trackB],
                adj_frame,
                self._nodes,
                row[:, :, 1],
    ...
    def fromSLEAP(cls, sleap_file, model_dict={}, model_yaml="", **kwargs):
        ...
        if model_dict and model_yaml:
            raise Exception("Only a single Interactor model may be specified")

        # Assign the model dict, if specified
        if model_yaml:
            interactor_model_dict = TrackingArray.loadYAML(model_yaml)
        elif model_dict:
            interactor_model_dict = model_dict

        return cls(
            frames_coords_array=frames_coords_array,
            tracks=tracks,
            nodes=nodes,
            interactor_model_dict=interactor_model_dict,
            **kwargs,
        )
    ...
    def processInteractionsFile(
        ...
        interactions_origin_dataframe = pd.read_csv(interactions_file, sep="\t")[
            expected_cols
        ]
        if limit_interactions:
            interactions_origin_dataframe = interactions_origin_dataframe[
                interactions_origin_dataframe["Interaction Name"].isin(
                    limit_interactions
                )
            ]
        ...
        interactions_origin_dataframe = interactions_origin_dataframe.drop(
            columns=["Destination interactor"]
        )
        interactions_origin_dataframe = interactions_origin_dataframe.rename(
            columns={"Origin interactor": "Instance"}
        )

        # Assign the destination instances for each frame
        interactions_dest_dataframe = interactions_dest_dataframe.drop(
            columns=["Origin interactor"]
        )
        interactions_dest_dataframe = interactions_dest_dataframe.rename(
            columns={"Destination interactor": "Instance"}
        )

        # Create the instance dataframe
        instance_dataframe = interactions_origin_dataframe.append(
            interactions_dest_dataframe, ignore_index=True
        )
        instance_dataframe = instance_dataframe[
            instance_dataframe["Interaction Frame"].between(frame_start, frame_end)
        ]

        # Check the presence of interactions within the given frames
        if instance_dataframe.shape[0] == 0:
            raise Exception(
                f"No interactions found between frames {frame_start} to {frame_end}"
            )

        instance_dataframe = (
            instance_dataframe.groupby("Interaction Frame")["Instance"]
            .apply(list)
