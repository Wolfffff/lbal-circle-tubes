...
for i in backbones:
    with open(
        "G:/My Drive/KL/SLEAP/11_16_23_LBAL_Model_Training/lbal_grid_search/centered_instance_"
        + i
        + ".json"
    ) as file:
    ...
    for f in filters:
        for ms in max_strides:
            for s in sigmas:
                ...
                update_run_name(
                    config,
                    f"backbone_{backbones[b]}_filters_{f}_max_stride_{ms}_sigma_{s}",
                )
                jsons.append(config)
                print(
                    f"Generated config with backbone = {backbones[b]}, filters = {f}, max_stride = {ms}, and sigma = {s}: {config}"
                )
