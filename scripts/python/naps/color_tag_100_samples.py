...
class ColorTagModel:
    ...
    def quadrantDetect(self, img):
        ...
        for i in range(4):
            ...
            if len(quad) > 150:
                quad = np.random.choice(quad, 150)
            elif len(quad) < 150:
                quad = np.append(quad, np.repeat(np.nan, (150 - len(quad))))
        ...
        top_prediction_percent = max(prediction_count_dict.values()) / sum(
            prediction_count_dict.values()
        )
        if top_prediction_percent < self.prediction_cutoff:
            prediction = [None]
        else:
            prediction = [max(prediction_count_dict, key=prediction_count_dict.get)]
    ...
    def sampleDetect(self, img):
        ...
        for sample_img in self.sampleArray(img, self.sample_count):
            ...
            if len(sample_img) > 150:
                sample_img = np.random.choice(sample_img, 150)
            elif len(sample_img) < 150:
                sample_img = np.append(
                    sample_img, np.repeat(np.nan, (150 - len(sample_img)))
                )
        ...
        if top_prediction_percent < self.prediction_cutoff:
            ...
            return [max(prediction_count_dict, key=prediction_count_dict.get)]
    ...
    def detect(self, img):
        ...
        img = img.flatten()
        if len(img) > 150:
            img = np.random.choice(img, 150)
        elif len(img) < 150:
            img = np.append(img, np.repeat(np.nan, (150 - len(img))))
    ...
    def sampleArray(array, n, sample_crop_size=1):
        ...
        while sample_count < n:
            ...
            if (cmin <= x_value <= cmax) and (cmin <= y_value <= cmax):
                ...
                sample_array = array[
                    (y_value - crop) : (y_value + crop),
                    (x_value - crop) : (x_value + crop),
                ]

                # Confirm the array has values
                if np.isnan(sample_array).all():
                    continue
